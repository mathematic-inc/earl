...
```sh
cargo binstall --disable-strategies compile earl
```
...
```toml
[tools]
"github:mathematic-inc/earl" = "latest"
```

Run `mise install` to download and activate the executable. No custom mise plugin
is required. The Cargo backend (`cargo:earl`) also supports these releases;
...
HTTP, GraphQL, gRPC, and SQL are included on every platform. Bash is included
on macOS and Linux. Browser support is included on macOS and Linux x64 GNU.
Local search is included on macOS ARM64 and Linux x64 GNU; ONNX Runtime does not
provide the libraries needed for the other release targets. Windows releases
retain the HTTP, GraphQL, gRPC, and SQL feature set.
...
Start with a [Discussion](https://github.com/mathematic-inc/earl/discussions/new),
