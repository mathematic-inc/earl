...
<Callout type="info">
  **Linux:** A Secret Service daemon is required (`gnome-keyring`, `kwallet`, or `keepassxc` with Secret Service enabled). If no daemon is running, `earl secrets set` will fail with a keyring error. See [Troubleshooting](/docs/troubleshooting) for setup guidance.
</Callout>
