...
`--yes` and `--json` are options on the `call` subcommand itself, so they must come before the command name:

```bash
# Correct
earl call --yes --json github.search_repos --q "rust"

# Wrong — flags after the command name are silently ignored
earl call github.search_repos --q "rust" --yes --json
```
...
| `--allow-unauthenticated` | off    | Skip authentication for the HTTP transport. Mutually exclusive with JWT configuration. Not recommended for production. |
