...
### `earl templates install`

Install a template from a local file path or a direct HTTP(S) URL to a `.hcl` file.

```bash
earl templates install <source> [--scope local|global] [--json]
```

| Flag      | Default | Description |
| --------- | ------- | ----------- |
| `--scope` | `local` | Install destination: `local` (./templates/) or `global` (~/.config/earl/templates/). |
| `--json`  | off     | Output install result as JSON. |

```bash
# Install from local file
earl templates install ./my-provider.hcl

# Install from URL into global scope
earl templates install https://example.com/templates/slack.hcl --scope global
```
...
<Callout type="warn">
  ...
  tool without `--yes`, the server returns an error:
  `tool 'X' is write-mode; restart the MCP server with --yes to enable write tools`.
</Callout>
