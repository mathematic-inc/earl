...
Define profiles under `[auth.profiles.<name>]` and reference them from templates via `auth.kind = "oauth2_profile"`. See [Auth](/docs/secrets-and-auth) for flow details.
