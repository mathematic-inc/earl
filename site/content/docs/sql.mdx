...
There is no `sandbox` block here. Write commands can't use `read_only = true`, and adding a sandbox without that flag would still block writes at the transaction level. The `RETURNING` clause is PostgreSQL-specific — adjust for your database if you're on MySQL or SQLite.
