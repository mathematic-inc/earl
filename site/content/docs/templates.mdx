---
title: Templates
description: Author, install, and manage HCL template files that define constrained API operations
icon: FileCode2
---
...
Install templates from a local file path or a direct HTTP(S) URL.
...
```bash
earl templates install ./my-template.hcl
earl templates install /opt/templates/github.hcl
```
...
```bash
earl templates install https://example.com/templates/github.hcl
```
...
By default, templates install to the local `./templates/` directory. Use `--scope global` to install to `~/.config/earl/templates/`:

```bash
earl templates install ./github.hcl --scope global
```
...
After installation, earl reports any secrets declared by the template:

```
Installed template from `./github.hcl` to `./templates/github.hcl`.
Required secrets:
- github.token
Install with:
...
- Installation fails if the destination file already exists.
