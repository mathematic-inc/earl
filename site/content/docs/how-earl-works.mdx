...
For more on writing templates: [Template Schema](/docs/template-schema).
...
Earl operates with two distinct trust levels.

**Templates are trusted.** Template files are committed to a version-controlled repository by human engineers with access controls applied. Earl executes what the template says unconditionally. If an attacker can write to that repository, they control what the agent can do.

**Arguments are untrusted.** Arguments come from the AI agent, which may have processed arbitrary external content — API responses, user messages, document contents. An agent that has been subject to prompt injection may pass malicious values for any declared parameter.

The sandbox (bash), read-only transactions (SQL), and SSRF blocking are the mechanisms that make untrusted arguments workable. They limit what a malicious argument value can reach even when the template author didn't anticipate every possible input. Template authors are responsible for not rendering string arguments directly into positions where the shell or database engine would interpret them — see [Hardening](/docs/hardening) for the safe patterns.
...
**SQL injection via direct query interpolation.** Parameterized queries protect values in the `params` array. The `query` string itself is rendered by Earl's template engine before it reaches the driver, so an arg interpolated directly into `query` — rather than into `params` — is raw SQL. Dynamic table or column names are the common case where this matters, since SQL doesn't allow parameterizing identifiers. Restrict those to an explicit allowlist in the template. Earl doesn't protect against a misconfigured driver that doesn't actually parameterize bound parameters either.
...
- [Template Schema](/docs/template-schema) — how to build templates that use these controls
