...
OpenClaw is a rapidly growing open-source agent platform with hundreds of thousands of users. In February 2026, security firm Koi Security found 341 malicious skills published to ClawHub, OpenClaw's community library. Those skills embedded supply-chain malware — the Atomic Stealer infostealer on macOS — inside their install dependencies. Separately, the instruction model itself creates a structural vulnerability: ClawHub skills are Markdown files that the LLM reads as instructions. An attacker-published skill can embed arbitrary instructions in that content, and the model will follow them — not because the model is broken, but because that's what text-in-context means.
