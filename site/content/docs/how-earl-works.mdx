...
OpenClaw is a widely-used agent platform with around 400,000 users. In February 2026, security researchers found 341 malicious "skills" published to ClawHub, OpenClaw's community library. Two CVEs followed: CVE-2026-25253 (1-click RCE) and CVE-2026-25157 (command injection). Both worked through the same mechanism: OpenClaw's skills are Markdown files that the LLM reads and executes. An attacker can embed instructions in skill content, in API responses the agent processes, or in tool descriptions themselves. The LLM can be told to "ignore previous instructions" and it sometimes listens — not because the model is broken, but because that's what text-in-context means.
...
When an agent calls Earl, it sees a tool name, a summary, and parameter names. That's it. The actual operation — the URL, method, headers, auth setup, request body — lives in an HCL file on disk. The agent cannot access it. It is not in the tool description, not in the MCP metadata, not passed back in any response.
...
The `result.output` block in a template has no access to `secrets.*`. You cannot accidentally or intentionally write `{{ secrets.api_key }}` and have it expand.
...
Templates annotated with `mode = "write"` pause and ask for confirmation before the request goes out:
